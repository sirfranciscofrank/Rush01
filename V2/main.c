...
/*
1. main()            [main.c]      ← OS starts here
2. ft_init_grid()    [main.c]      first thing main does (empty the grid)
3. ft_parse()        [ft_parse.c]  read the clues from argv[1]

4. ft_solve()        [ft_solve.c]  ← THE MISSING MIDDLE (does all the work)
     for each cell, try values 1-4:
       ├─ ft_is_safe()        [ft_rules.c]   value not repeated in row/col?
       └─ ft_check_partial()  [ft_solve.c]   line full → clues match?
            ├─ ft_check_row()  [ft_rules.c] ─→ ft_count_left/right()  [ft_views.c]
            └─ ft_check_col()  [ft_rules.c] ─→ ft_count_top/bottom()  [ft_views.c]
       (then recurse to the next cell, or undo and try again)

5. ft_print_grid()   [ft_print.c]  last worker on success
6. main return 0     [main.c]      ← LAST of all
*/
